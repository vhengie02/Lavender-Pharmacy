...
@section('title', 'Receipts - Lavender Pharmacy')

@section('content')
    <header class="mb-8 border-b border-border pb-4">
        <h1 class="font-serif text-2xl font-bold text-foreground">Receipts</h1>
        <p class="mt-1 text-sm text-muted-foreground">Issued receipts linked to orders.</p>
    </header>
                ...
                <tbody class="divide-y divide-border">
                    @forelse ($receipts as $receipt)
                        <tr>
                            ...
                            <td class="px-4 py-3">{{ $receipt->order?->user?->name ?? '—' }}</td>
                            <td class="px-4 py-3 text-muted-foreground">{{ $receipt->date_created?->format('M d, Y H:i') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center text-muted-foreground">No receipts found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
