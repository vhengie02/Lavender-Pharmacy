...
<header class="fixed top-0 left-0 right-0 z-50 border-b border-border bg-background/80 backdrop-blur-md">
    <nav class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div class="hidden items-center gap-3 md:flex lg:gap-4">
            <a href="tel:+1234567890" class="hidden shrink-0 items-center gap-2 text-sm text-muted-foreground transition-colors hover:text-primary xl:flex" title="(123) 456-7890">
                ...
                <span class="whitespace-nowrap">(123) 456-7890</span>
            </a>
            @auth
                <x-cart-nav-icon :count="$cartItemCount ?? 0" class="z-10" />
    ...
    <div id="mobile-nav" class="hidden border-t border-border md:hidden">
        <div class="space-y-1 px-2 pb-3 pt-2">
            ...
            @else
                <a href="{{ route('cart.index') }}" class="mt-2 flex items-center justify-center gap-2 rounded-lg bg-primary px-3 py-2 text-center text-sm font-medium text-primary-foreground">
                    Cart
                    @if (($cartItemCount ?? 0) > 0)
                        <span class="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-red-600 px-1.5 text-xs font-bold text-white">
                            {{ ($cartItemCount ?? 0) > 99 ? '99+' : $cartItemCount }}
                        </span>
                    @endif
                </a>
            @endguest
